...
const SERIES_INFO_URL =
  "http://api.sportradar.us/nba/trial/v5/en/series/2018/PST/schedule.json";
  ...
  convert: async (apiGamesSched, isNew, isRecent) => {
    try {
      ...
      for (game of apiGamesSched) {       
          ...
          game["totalBehindLeague"] =
            homeTeamBehindLeague + awayTeamBehindLeague;
          // fill playoff series info
          if (playoffs) {
            // console.log('playoffSeries: ' + JSON.stringify(playoffSeries));
            game["playoffSeries"] = findPlayoffInfoForGame(
              game,
              homeTeamName,
              awayTeamName,
              playoffSeriesInfo
...
function findPlayoffInfoForGame(
  game,
  homeTeamName,
  awayTeamName,
  playoffSeriesInfo
  ...
  for (serie of playoffSeriesInfo.apiData.series) {
    if (
      serie.participants[0].name === homeTeamName ||
      serie.participants[0].name === awayTeamName
    ) {
...
async function fetchTeamStats() {
  try {
    const dbTeamList = await Team.find({}).select(
      "srId name gamesBehind.league"
    );
    if (dbTeamList) {
      return dbTeamList;
    } else {
...
async function fetchPlayoffSeriesInfo() {
  try {
    const seriesInfo = await fetchFromApi.fetchData(
      `${SERIES_INFO_URL}?api_key=${keys.SPORTRADAR_API_KEY}`
    );
    return seriesInfo;
  } catch (error) {
    console.log(error);
