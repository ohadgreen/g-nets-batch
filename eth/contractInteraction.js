...
const web3js = new Web3(new Web3.providers.HttpProvider(keys.INFURA_ENDPIONT));
// const contract = new web3js.eth.Contract(JSON.parse(interface), CONTRACT_ADDRESS);
const contract = new web3js.eth.Contract(JSON.parse(keys.INTERFACE), keys.CONTRACT_ADDRESS);

const callContract = async () => {        
    const contractBalance = await contract.methods.showTotalBetSum().call();
    console.log('balance: ' + contractBalance);

    const txnDetails = await placeBetDetails('0.03', keys.JORDAN_ADDRESS, keys.JORDAN_INT_CODE);
